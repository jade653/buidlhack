...
SUBMISSION_DIR = ROOT / "samples" / "cosmetics_submission"
CHALLENGE_INPUT = ROOT / "samples" / "cosmetics_challenge_input.json"
OUTPUT_HTML = ROOT / "samples" / "generated" / "luma-dew-storefront.html"
ENV_FILE = ROOT / ".env"
...
def main() -> int:
    ...
    api_key = os.environ.get("NEAR_AI_API_KEY", "").strip()
    if not api_key:
        print("[ERROR] NEAR_AI_API_KEY is not set.")
        print(
            f'Set it in {ENV_FILE} or export it first, for example: '
            'export NEAR_AI_API_KEY="your-key"'
        )
        return 1
